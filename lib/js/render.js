...
export function renderStartpage(root, app) {
    ...
    root.querySelector("[data-theme-select]").addEventListener(
        "change",
        (event) => {
...
function renderTheme(root, state, themeId) {
    ...
    icon.className =
        targetTheme.variant === "dark" ? "i-ri-moon-fill" : "i-ri-sun-fill";
...
function getOppositeTheme(state, theme) {
    ...
    return (
        state.themes.find(
            (item) =>
                item.family === theme.family && item.variant === oppositeVariant,
        ) ?? theme
    );
}
...
function renderThemeList(root, state, activeThemeId) {
    ...
    const activeTheme =
        state.themes.find((theme) => theme.id === activeThemeId) ??
        state.themes[0];
    ...
    swatch.replaceChildren(
        ...["base08", "base0A", "base0B", "base0D"].map((key) => {
            ...
            color.style.background = activeTheme.palette[key];
            return color;
        }),
    );
}
