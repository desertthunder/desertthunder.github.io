...
import { attachApp } from "../render.js";
...
describe("attachApp", () => {
	it("renders browser, theme, and default links", () => {
		...
		expect(root.textContent).toContain("Meta");
	...
	it("renders city autocomplete options from typed input", () => {
		...
		expect([...root.querySelectorAll(":scope [data-city-options] option")].map((option) => option.value)).toContain(
			"Chicago, IL",
		);
	...
	it("adds and removes links through the rendered form", () => {
		...
		root.querySelector(':scope [data-category="Search"][data-index="0"]').click();
	...
	it("renders link categories as a select and renames only custom categories", () => {
		...
		expect(root.querySelector('[data-category-rename-form][data-category="Development"]')).toBeFalsy();
	...
	it("resets settings from the sidebar", () => {
		const { app, root, stop } = createApp();
		const form = root.querySelector("[data-link-form]");
		...
		root.querySelector('[data-action="reset-settings"]').click();

		expect(app.state.city.value).toBe("Austin");
		expect(app.state.theme.value).toBe("gruvbox-dark-hard");
		expect(app.state.links.value.Search).toBeUndefined();
		expect(app.state.links.value.Meta).toBeDefined();
		expect(app.actions.refreshWeather).toHaveBeenCalled();
